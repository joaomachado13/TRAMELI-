(() => {
  ...
  const readOrders = () => { const value = readJson(orderKey, []); return Array.isArray(value) ? value : []; };
  const profile = readJson(profileKey, {});
  let token = localStorage.getItem(tokenKey);
  if (!token) { token = crypto.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2)}`; try { localStorage.setItem(tokenKey, token); } catch { /* Demo continues without persistence. */ } }
  ...
  const photo = product => product.image ? `<img src="${escapeHtml(product.image)}" alt="${escapeHtml(product.name)}" loading="lazy">` : '<span class="portal-photo-empty" aria-hidden="true">✳</span>';
  ...
  function placeOrder(form) {
    ...
    const previous = oldOrders.find(order => order.id === editingOrderId && order.customerToken === token && !order.checked);
  ...
  host.addEventListener('click', event => {
    ...
    if (editButton) {
      const order = readOrders().find(item => item.id === editButton.dataset.editOrder && item.customerToken === token && !item.checked);
      if (!order) { render(); return; }
      cart = {};
      order.items.forEach(item => { const product = products().find(entry => entry.name === item.name); if (product) cart[product.id] = item.quantity; });
    ...
    if (cancelButton) {
      const oldOrders = readOrders();
      const order = oldOrders.find(item => item.id === cancelButton.dataset.cancelOrder && item.customerToken === token && !item.checked && item.date >= dateKey(new Date()));
      if (!order || !confirm('Cancelar este pedido?')) return;
      try { localStorage.setItem(orderKey, JSON.stringify(oldOrders.filter(item => item.id !== order.id))); }
